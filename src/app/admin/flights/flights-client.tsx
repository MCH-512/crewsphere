...
export function FlightsClient({ initialFlights, initialUsers }: { initialFlights: FlightForDisplay[], initialUsers: User[] }) {
    ...
    const [userMap, setUserMap] = React.useState<Map<string, User>>(() => new Map(initialUsers.map(u => [u.uid, u])));
    const [pursers, setPursers] = React.useState<User[]>(() => initialUsers.filter(u => ['purser', 'admin', 'instructor'].includes(u.role || '') && u.role !== 'pilote'));
    const [pilots, setPilots] = React.useState<User[]>(() => initialUsers.filter(u => u.role === 'pilote'));
    const [cabinCrew, setCabinCrew] = React.useState<User[]>(() => initialUsers.filter(u => u.role === 'cabin crew'));
    const [instructors, setInstructors] = React.useState<User[]>(() => initialUsers.filter(u => u.role === 'instructor'));
    const [trainees, setTrainees] = React.useState<User[]>(() => initialUsers.filter(u => u.role === 'stagiaire'));
