...
export default function AdminFlightsPage() {
  ...
  const getStatusBadgeVariant = (status: Flight["status"]) => {
    switch (status) {
      ...
      case "On Time": return "default"; // Often green-ish or primary
      case "Delayed": return "outline"; // Could be yellow if theme supports
      case "Cancelled": return "destructive";
      default: return "secondary";
    }
  };

  const getStatusBadgeClassName = (status: Flight["status"]) => {
    switch (status) {
        case "Scheduled": return "bg-blue-100 text-blue-700 border-blue-300";
        case "On Time": return "bg-green-100 text-green-700 border-green-300";
                    ...
                    <TableRow key={flight.id}>
                      ...
                      <TableCell>
                        <Badge 
                            ...
                            className={getStatusBadgeClassName(flight.status)}
                        >
                          {flight.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right space-x-2">
                        <Button variant="ghost" size="sm" onClick={() => toast({ title: "Edit Flight", description: "Editing functionality coming soon!"})} disabled>
                          <Edit className="mr-1 h-4 w-4" /> Edit
                        </Button>
                         <Button variant="ghost" size="sm" onClick={() => toast({ title: "Delete Flight", description: "Deletion functionality coming soon!"})} disabled className="text-destructive hover:text-destructive/80">
