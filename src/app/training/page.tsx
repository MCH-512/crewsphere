...
export default function TrainingHubPage() {
  ...
  const [trainingCourses, setTrainingCourses] = React.useState<CombinedCourse[]>([]);
  ...
  const fetchTrainingData = React.useCallback(async () => {
    ...
    try {
      ...
      const relevantCourses: CombinedCourse[] = [];
      ...
      for (const courseData of fetchedCoursesData) {
        ...
        if (progressDocSnap.exists()) {
          userProgress = { ...userProgress, ...progressDocSnap.data() } as UserProgressData;
        }
      ...
      setTrainingCourses(relevantCourses);
    } catch (err) {
      console.error("Error fetching training data:", err);
      setLoadingError("Failed to load your training. Please try again.");
  ...
  if (authLoading || isLoadingCourses) {
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <p className="ml-4 text-lg text-muted-foreground">Loading your training hub...</p>
      </div>
    );
  }
  ...
  if (!user && !authLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center">
        ...
        <p className="text-muted-foreground">Please log in to access your Training Hub.</p>
  ...
  if (loadingError) {
     return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center">
        <AlertTriangle className="h-12 w-12 text-destructive mb-4" />
        <CardTitle className="text-xl mb-2">Error Loading Training Hub</CardTitle>
    ...
    <div className="space-y-8">
      <Card className="shadow-lg">
        <CardHeader className="flex flex-row items-center gap-3">
          ...
          <div>
            <CardTitle className="text-2xl font-headline">My Active & Required Training</CardTitle>
            <CardDescription>
        ...
        <CardContent>
           <p className="text-muted-foreground">
             Focus on what's next. For a full list of all available courses, visit the <Link href="/courses" className="text-primary hover:underline">Course Library</Link>.
           </p>
        </CardContent>
      </Card>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              For now, choose to simulate passing or failing. A passing score is typically 80% or more.
            </DialogDescription>
          </DialogHeader>
